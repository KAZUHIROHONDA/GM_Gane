...
void Player::Draw()
{
	...
	status = "名前:" + Name;
	g_directWrite->DrawString(status,XMFLOAT2(50,50),D2D1_DRAW_TEXT_OPTIONS_NONE);
	oss << HP;
	status = "体力:"+ oss.str();
	g_directWrite->DrawString(status, XMFLOAT2(50, 100), D2D1_DRAW_TEXT_OPTIONS_NONE);
	gu << GUat;
	status = "グー:" + gu.str();
	g_directWrite->DrawString(status, XMFLOAT2(50, 150), D2D1_DRAW_TEXT_OPTIONS_NONE);
	tyo << TYOKIat;
	status = "チョキ:" + tyo.str();
	g_directWrite->DrawString(status, XMFLOAT2(50, 200), D2D1_DRAW_TEXT_OPTIONS_NONE);
	pa << PAat;
	status = "パー:" + pa.str();
	g_directWrite->DrawString(status, XMFLOAT2(50, 250), D2D1_DRAW_TEXT_OPTIONS_NONE);
